"""Bump the KreuzbergParser version following semantic-versioning rules.

The single source of truth is ``APP_VERSION`` in ``main.py``. This script
reads it, increments the requested component (default: patch) and writes the
new value back to ``main.py``, ``kreuzberg_parser_pynsist.cfg`` and
``pyproject.toml`` so the built installer always carries the bumped version.

Usage:
    python bump_version.py [patch|minor|major]

On success the new version is printed to stdout (so the build script can
capture it) and the exit code is 0.
"""
...
SOURCE_FILE = os.path.join(HERE, "main.py")
CONFIG_FILE = os.path.join(HERE, "kreuzberg_parser_pynsist.cfg")
...
def main():
    ...
    if not match:
        sys.stderr.write(
            'ERROR: could not find APP_VERSION = "x.y.z" in main.py\n'
        )
        return 1
    ...
    # Update kreuzberg_parser_pynsist.cfg. Replace only the Application
    # 'version=' line (the one equal to the current app version) so the
    # [Python] version is never touched.
    ...
    if not replaced:
        sys.stderr.write(
            "ERROR: could not find 'version={}' in kreuzberg_parser_pynsist.cfg\n".format(
                old_version
            )
        )
        return 1
