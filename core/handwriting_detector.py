...
class HandwritingDetector:
    ...
    def __init__(self, config: Config | None = None):
        ...
        # Typed as Any: transformers stubs are incomplete for generate()/to().
    ...
    def load_model(self) -> bool:
        ...
        if not self.enabled or TrOCRProcessor is None or VisionEncoderDecoderModel is None:
    ...
    def detect_handwriting_in_image(self, image_path: str) -> Dict[str, Any]:
        """
    ...
    def _extract_text_from_image(self, image: PILImage) -> str:
        """
    ...
    def _estimate_confidence(self, text: str) -> float:
        ...
        word_score = sum(
            1 for word in common_words if word in text.lower()
        ) / len(common_words)
    ...
    def enhance_page_with_handwriting_detection(
        self,
        ...
        if (
            hw_result["has_handwriting"]
            and hw_result["confidence"]
            > self.config.HANDWRITING_CONFIDENCE_THRESHOLD
        ):
            if hw_result["text"]:
                page_data["text"] += (
