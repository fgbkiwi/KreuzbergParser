"""
PaddleOCR on CUDA via Python onnxruntime-gpu (RapidOCR / PP-OCR ONNX).

Kreuzberg 4.10.2 ships a CPU-only bundled ORT (`ort-bundled`) and skips
ORT_DYLIB_PATH, so AccelerationConfig(provider="cuda") cannot work. This
module runs the same PaddleOCR family of ONNX models on the GPU ORT that
EasyOCR/PyTorch already share (cu130).
"""
...
def _png_to_rgb(png_bytes: bytes) -> np.ndarray:
    ...
    if bgr is None:
        raise ValueError("Could not decode PNG bytes for PaddleOCR GPU")
    return cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)


def get_paddle_gpu_engine(*, rec_batch_num: int = 8):
    ...
    with _engine_lock:
        ...
        logger.info(
            "Starting PaddleOCR GPU (RapidOCR PP-OCRv5 latin + onnxruntime-gpu CUDA)"
        )
...
def ocr_png_bytes(
    ...
    *,
    ...
    engine = get_paddle_gpu_engine(rec_batch_num=rec_batch_num)
