...
Otimizado para **PDFs gerados pelo PJe-JT**: cabeçalhos e rodapés de petições são excluídos; as linhas de assinatura digital do PJe são mantidas **apenas na última página** de cada documento do PDF, reduzindo tokens quando o Markdown é processado por um LLM.
...
### ⚖️ Otimização para PJe-JT

O pipeline é afinado para PDFs exportados pelo **PJe-JT**:

- **Cabeçalhos e rodapés** de petições são removidos do Markdown
- **Assinaturas digitais do PJe** são preservadas só na **última página** de cada documento dentro do PDF
- Objetivo: menos ruído repetido e **economia de tokens** quando o `.md` for consumido por um LLM

### 📦 Conversão em batch

**Batch** (ou conversão em lote) significa processar **vários PDFs de uma vez**, em sequência, sem precisar abrir e converter cada arquivo individualmente.

- Monte uma **fila** com um ou mais PDFs (botão **Selecionar PDFs**)
- Defina a pasta de destino, o modo de OCR e as opções
- Clique em **PROCESSAR FILA**: cada PDF da fila é convertido para Markdown, um após o outro
- Ao final, a UI mostra o resumo (sucesso, falhas e tempo total)

Útil quando há muitos autos ou peças para converter no mesmo turno de trabalho.
...
1. Baixar instalador: [https://github.com/UB-Mannheim/tesseract/wiki](https://github.com/UB-Mannheim/tesseract/wiki)
...
1. **Selecionar PDFs**: Clique em "📂 Selecionar PDFs" e monte a fila (um ou vários arquivos)
